...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	// all values in grid should add up to one

  vector< vector <float> > newGrid;

	// your code here
  float sz = grid.size() * grid.size(); 
	...
	float p_miss) 
{
  ...
  for (int i = 0; i < grid.size(); i++){
    ...
    for (int j = 0; j < grid[0].size(); j++){
      ...
      if (grid[i][j] == color){ // give value based from color 
            temp_vec.push_back(beliefs[i][j] * p_hit); 
      }
      else{
            temp_vec.push_back(beliefs[i][j] * p_miss);
      }
	...
	float blurring) 
{
  ...
  for (int i = 0; i < beliefs.size(); i++){
    for (int j = 0; j < beliefs[0].size(); j++){
      if(beliefs[i][j] == 1){
        newGrid[((i-1)-dx%3)][((j-1)-dy%3)] = 1; //find new position based from old 
      }
    }
